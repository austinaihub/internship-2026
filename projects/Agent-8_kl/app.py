...
with tab_control:
    ...
    if st.session_state.agent_state:
        ...
        if status == "approving_audience":
            st.warning("🎯 Action Required: Review Target Audience Selection")
            
            selected_audience = current_state.get("target_audience", "general_public")
            audience_brief = current_state.get("audience_brief", "")
            visual_style = current_state.get("visual_style", "")
            
            st.markdown(f"### Selected Audience: `{selected_audience}`")
            
            with st.expander("📝 Writing Brief (for Writer Agent)", expanded=True):
                st.write(audience_brief)
            
            with st.expander("🎨 Visual Style Brief (for Image Generator)", expanded=True):
                st.write(visual_style)
            
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ Approve Audience & Continue"):
                    config = {"configurable": {"thread_id": st.session_state.thread_id}, "run_name": f"Agent-8 Campaign {st.session_state.thread_id}"}
                    st.session_state.app.update_state(config, {"status": "audience_approved"})
                    for event in st.session_state.app.stream(None, config, stream_mode="values"): pass
                    st.session_state.agent_state = st.session_state.app.get_state(config).values
                    st.rerun()
            with col2:
                audience_options = ["college_students", "educators", "business_owners", "parents", "lawmakers", "general_public"]
                override = st.selectbox("Override audience:", audience_options, index=audience_options.index(selected_audience) if selected_audience in audience_options else 5)
                if st.button("🔄 Re-analyze with Override"):
                    config = {"configurable": {"thread_id": st.session_state.thread_id}, "run_name": f"Agent-8 Campaign {st.session_state.thread_id}"}
                    st.session_state.app.update_state(config, {"target_audience": None, "audience_brief": None, "visual_style": None, "status": "override_audience", "feedback": override})
                    for event in st.session_state.app.stream(None, config, stream_mode="values"): pass
                    st.session_state.agent_state = st.session_state.app.get_state(config).values
                    st.rerun()

        # --- PROMPT APPROVAL STATE ---
        elif status == "approving_prompt":
        ...
        elif status not in ["approving_audience", "approving_prompt", "approving_text", "approving_image", "error", "done"]:
