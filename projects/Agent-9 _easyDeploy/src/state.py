...
class AgentState(TypedDict):
    ...
    text_feedback: Optional[str]        # User feedback for refining post text (refinement loop)
    audience_feedback: Optional[str]    # User feedback for re-targeting audience (refinement loop)
    feedback: Optional[str]             # For HITL loops
    user_search_keywords: Optional[str] # User-provided keywords for targeted Exa search (optional)
    user_guidance: Optional[str]        # User creative direction for downstream agents (from trend review HITL)
